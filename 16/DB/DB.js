...
function DB(cb) {
  this.getFaculties = (args, context) => {
    console.log("DB GET FACS");
    return new mssql.Request().query("select * from FACULTY").then((r) => {
      return r.recordset ? r.recordset : false;
    });
  };

  this.getPulpits = (args, context) => {
    return new mssql.Request().query("select * from PULPIT").then((r) => {
      return r.recordset ? r.recordset : false;
    });
  };

  this.getSubjects = (args, context) => {
    return new mssql.Request().query("select * from SUBJECT").then((r) => {
      return r.recordset;
    });
  };

  this.getTeachers = (args, context) => {
    return new mssql.Request().query("select * from TEACHER").then((r) => {
      return r.recordset;
    });
  };

  this.getFaculty = (args, context) => {
    console.log("DB GET FAC");
  ...
  this.insertSubject = (args, context) => {
    return new mssql.Request()
      ...
      .query(
        "insert SUBJECT(SUBJECT, SUBJECT_NAME, PULPIT) values (@a, @b, @c)"
      )
      .then((r) => {
        return args;
      });
  ...
  this.insertTeacher = (args, context) => {
    return new mssql.Request()
      ...
      .input("c", mssql.NVarChar, args.PULPIT)
      .query(
        "insert teacher(TEACHER, TEACHER_NAME, PULPIT) values (@a, @b, @c)"
      )
      .then((r) => {
        return args;
      });
  ...
  this.updateFaculty = (args, context) => {
    return new mssql.Request()
      ...
      .input("b", mssql.NVarChar, args.FACULTY_NAME)
      .query(
        "update FACULTY set FACULTY = @a, FACULTY_NAME = @b where FACULTY = @a"
      )
      .then((r) => {
  ...
  this.updatePulpit = (args, context) => {
    return new mssql.Request()
      ...
      .input("c", mssql.NVarChar, args.FACULTY)
      .query(
        "update PULPIT set PULPIT = @a, PULPIT_NAME = @b, FACULTY = @c where PULPIT = @a"
      )
      .then((r) => {
        return r.rowsAffected[0] === 0 ? null : args;
      });
  ...
  this.updateSubject = (args, context) => {
    return new mssql.Request()
      ...
      .input("c", mssql.NVarChar, args.PULPIT)
      .query(
        "update SUBJECT set SUBJECT = @a, SUBJECT_NAME = @b, PULPIT = @c where SUBJECT = @a"
      )
      .then((r) => {
        return r.rowsAffected[0] === 0 ? null : args;
      });
  ...
  this.updateTeacher = (args, context) => {
    return new mssql.Request()
      ...
      .input("c", mssql.NVarChar, args.PULPIT)
      .query(
        "update TEACHER set TEACHER = @a, TEACHER_NAME = @b, PULPIT = @c where TEACHER = @a"
      )
      .then((r) => {
        return r.rowsAffected[0] === 0 ? null : args;
      });
    ...
    return new mssql.Request()
      ...
      .then((r) => {
        ...
        r.recordset.forEach((el, index) => {
          ...
          else if (rc[rc.length - 1].FACULTY !== el.FACULTY) rc.push(zapp(el));
          else rc[rc.length - 1].TEACHERS.push(zaps(el));
        });
        console.log(rc);
        return rc;
      });
  ...
  this.getSubjectsByFaculties = (args, context) => {
    return new mssql.Request()
      ...
      .then((r) => {
        ...
        r.recordset.forEach((el, index) => {
          if (index === 0) rc.push(zapp(el));
          else if (rc[rc.length - 1].PULPIT !== el.PULPIT) rc.push(zapp(el));
